...
/proc/HandleScomJoinFor(var/mob/living/carbon/L, var/datum/game_mode/scom/caller)
	...
	if(L.job == "Captain")
		...
		//end workaround
		L << ("\red You are the Commander of the S-COL forces. You are expected to control all local aspects of your S-COL base, research, medical, supply and tactical. You should not attend combat missions yourself, unless you have no other option. Your goal is to ensure the project runs smoothly. You report only to the Council and its members. Good luck, the fate of the galaxy rests on your frail shoulders.")
...
*/
		...
		//end workaround
		L << ("\blue You are leading squad [squadpick]")
		...
		//end workaround
		L << ("\blue You are in squad [squadpick]")
		L << ("\red You are the backbone of the S-COM project. The operatives. Divided into four classes (Combat Medic, Assault, Heavy, Sniper), you are the last and greatest line of defence against the alien menace. You report to your squad leaders and then to the commander. Good luck soldier, the fate of the galaxy rests on your frail shoulders.")
	L.regenerate_icons()
