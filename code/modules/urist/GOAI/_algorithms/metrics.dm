...
# define CHEBYSHEV_DISTANCE_THREED(from_atom, to_atom, DEFAULT, ZMULT) ( (isnull(from_atom) || isnull(to_atom) ) ? DEFAULT : CHEBYSHEV_DISTANCE_NUMERIC_TWOD(from_atom.x, from_atom.y, to_atom.x, to_atom.y) + (ZMULT * abs(to_atom.z - from_atom.z)) )
