...
/obj/item/weapon/grenade/sonic/prime()
	...
	for(var/mob/living/carbon/M in hearers(7, src.loc))//get_turf(src)
		...
		if(ishuman(M))
			if(M:is_on_ears(/obj/item/clothing/ears/earmuffs))
				safety = 1
			if(M:is_on_ears(/obj/item/clothing/ears/earmuffs/earplugs/syndiplugs))
				syndi_safety = 1			
		...
		if(syndi_safety == 1)
			...
			if(distance <= 4) //Middle section				
				M.Weaken(4/(safety + 1))
				M.Stun(6/(safety + 1))
				M.ear_deaf = (20/(safety + 1))
				M.stuttering = (20)
				M.make_jittery(100)
			else if(distance <= 1) //Right next to it
				M.Stun(8/(safety + 1))
				M.Weaken(6/(safety + 1))
				M.ear_deaf = (30/(safety + 1))
				M.stuttering = (30)
				M.make_jittery(150)
			else //Far away		
				M.Stun(4/(safety + 1))	
				M.Weaken(2/(safety + 1))
				M.ear_deaf = (10/(safety + 1))
				M.stuttering = (10)
...
/obj/item/weapon/storage/box/sonics
	...
	desc = "A box with 5 sonic grenades and earplugs."
	...
	New()
		...
		new /obj/item/weapon/grenade/sonic(src)
		new /obj/item/weapon/grenade/sonic(src)
		new /obj/item/clothing/ears/earmuffs/earplugs/syndiplugs(src)
