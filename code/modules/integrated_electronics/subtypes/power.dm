...
/obj/item/integrated_circuit/power/transmitter
	...
	extended_desc = "This circuit transmits up to 10 kJ of electricity every time the activator pin is pulsed. The input pin must be \
	...
	inputs = list(
		...
		"joules per activation" = IC_PINTYPE_NUMBER
		)
	outputs = list(
...
/obj/item/integrated_circuit/power/transmitter/large
	...
	extended_desc = "This circuit transmits up to 40 kJ of electricity every time the activator pin is pulsed. The input pin must be \
	a reference to a machine to send electricity to. This can be a battery, or anything containing a battery. The machine can exist \
	inside the assembly, or adjacent to it. The power is sourced from the assembly's power cell. If the target is outside of the assembly, \
...
/obj/item/integrated_circuit/power/transmitter/do_work()
	...
	if(cell)
		...
		if(get_dist(A,B) < 8)
			if(A != B)
				transfer_amount *= 0.9 // Losses due to distance.
			var/list/U=filter_list(A.GetAllContents(),/obj/item/integrated_circuit/power/transmitter)
			for(var/I = U.len to 1)
				transfer_amount *= 0.825
		...
		if(get_dist(get_turf(src),get_turf(S)) < 8)
			set_pin_data(IC_OUTPUT, 1, S.charge)
			set_pin_data(IC_OUTPUT, 2, S.capacity)
			set_pin_data(IC_OUTPUT, 3, S.Percentage())
			activate_pin(2)
			push_data()
			if(S.charge == S.capacity)
				return FALSE
			if(assembly.draw_power(amount_to_move))
				S.add_charge(amount_to_move)
				return TRUE
	else
