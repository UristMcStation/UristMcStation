...
/datum/ai_holder
	...
	var/astar_adjacent_proc = TYPE_PROC_REF(/turf, CardinalTurfsWithAccessWithZ) // Proc to use when A* pathfinding.  Default makes them bound to cardinals.
...
/datum/ai_holder/proc/get_path(turf/target,get_to = 1, max_distance = world.view*6)
	...
	forget_path()
	var/list/new_path = AStar(get_turf(holder.loc), target, astar_adjacent_proc, TYPE_PROC_REF(/turf, Manhattan3dDistance), min_target_dist = get_to, max_node_depth = max_distance, id = holder.IGetID(), exclude = obstacles)
