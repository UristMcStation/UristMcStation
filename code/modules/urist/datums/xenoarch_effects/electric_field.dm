...
/datum/artifact_effect/electric_field/DoEffectTouch(mob/living/user)
	...
	for (var/mob/living/L in nearby_mobs)
		...
		if (ishuman(L))
			...
			L.electrocute_act(rand(25, 40), holder, 0.75, BP_CHEST)
...
/datum/artifact_effect/electric_field/DoEffectAura()
	...
	for (var/mob/living/L in nearby_mobs)
		...
		if (ishuman(L))
			...
			L.electrocute_act(rand(1, 10), holder, 0.75, BP_CHEST)
...
/datum/artifact_effect/electric_field/DoEffectPulse()
	...
	for (var/mob/living/L in nearby_mobs)
		...
		if (ishuman(L))
			...
			L.electrocute_act(rand(10, 30), holder, 0.75, BP_CHEST)
