...
//	if(!M) return
	...
	switch(dresscode)
		...
		if("assassin")
			...
			M.equip_to_slot_or_del(new /obj/item/clothing/accessory/wcoat(M), slot_tie)
		...
		if("ANTAG operative")
			...
			sec_briefcase.contents += new /obj/item/ammo_magazine/mc9mm
			sec_briefcase.contents += new /obj/item/weapon/grenade/empgrenade(src) //anti-tank :D
...
//			W.access += get_all_centcom_access()
		...
		if("RDF Soldier")
			...
			M.equip_to_slot_or_del(new /obj/item/device/flashlight(M), slot_in_backpack)
			M.equip_to_slot_or_del(new /obj/item/weapon/plastique(M), slot_in_backpack)
			M.equip_to_slot_or_del(new /obj/item/weapon/gun/projectile/automatic/kh50(M), slot_in_backpack)
