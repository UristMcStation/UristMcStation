...
1.  Firstly, define two markers for each submap. The types need to be unique, and
    ...
    ```dm
    /obj/urist_intangible/mapmanip/marker/submap/extract/station/boxstation/warehouse_small_storage
    	name = "Boxstation, Cargo Warehouse Small Storage"

    /obj/urist_intangible/mapmanip/marker/submap/insert/station/boxstation/warehouse_small_storage
    	name = "Boxstation, Cargo Warehouse Small Storage"
    ```

2.  If you're making submaps for a baked in map, like Nerva or Glloydstation, 
    you have to do things slightly differently. Otherwise, just go straight to 2b. 
    Instead of adding an insert marker on the map, make a template_loader variant, 
    and a /datum/map_template/submap variant for the submap that you want to load. 
    Then, put the template_loader where you otherwise would have put the insert marker.
    Make a new dmm file that is the same size as the submap that you want to make 
    (i.e. 3x5 following the given example), and put the insert marker there instead.

    2b. Figure out the area that the submap will be inserted into on the map, and add
    the insert marker there. In this case the map is `maps/stations/boxstation.dmm`, 
    and the size of that area is 3x5 tiles, being the little side warehouse. Here, 
    we want the submaps to only be the "cargo" or "contents" of the warehouse. 
    So this room is already fully functional, with lights, cameras, pipes, etc. 
    Do note that the "submap edge" helper markers are just visual helpers, and they have no function.
    ...
    [
    	{
    		...
    		"marker_extract": "/obj/urist_intangible/mapmanip/marker/submap/extract/station/boxstation/warehouse_small_storage",
    		"marker_insert": "/obj/urist_intangible/mapmanip/marker/submap/insert/station/boxstation/warehouse_small_storage",
...
If a map manipulation fails, it should do return `null`, and in that case the map is loaded without any map
