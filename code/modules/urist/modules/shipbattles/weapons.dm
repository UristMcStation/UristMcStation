...
/obj/item/shipweapons/torpedo_warhead/attackby(var/obj/item/I, mob/user as mob)
	if(istype(I, /obj/item/weapon/crowbar))
		...
		if(!riggedstate)
			return
		to_chat(user, "<span class='notice'>You begin to [safety ? "disable" : "re-engage"] the built-in safeties.</span>")
		if(do_after(user,4 SECONDS))
			playsound(src.loc, 'sound/machines/buttonbeep.ogg', 25, 0, 10)
			to_chat(user, "<span class='notice'>You [safety ? "disable" : "re-engage"] the built-in safeties.</span>")
			safety = !safety
			icon_state = "torpedowarhead-open-mod[safety ? "" : "-armed"]"
