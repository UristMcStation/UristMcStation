...
/*	var/area/start_location = locate(/area/jungle/crash_ship_source)
	...
	while(num_spawned < max)
		var/turf/simulated/planet/jungle/J = locate(rand(RANDOM_LOWER_X, RANDOM_UPPER_X), rand(RANDOM_LOWER_Y, RANDOM_UPPER_Y), src.z)
		if(!istype(J))
			continue
	...
	for(var/obj/effect/landmark/river_waypoint/W in landmarks_list)
		...
		W.connected = 1
		var/turf/cur_turf = new /turf/simulated/planet/jungle/water(get_turf(W))
		...
		while(cur_turf != target_turf)
			...
			var/skip = 0
			if(!istype(cur_turf, /turf/simulated/planet/jungle) || istype(cur_turf, /turf/simulated/planet/jungle/rock))
				detouring = 0
				cur_dir = get_dir(cur_turf, target_turf)
				cur_turf = get_step(cur_turf, cur_dir)
				continue
			...
			if(!skip)
				var/turf/simulated/planet/jungle/water/water_turf = new(cur_turf)
...
/*	max = rand(2,5)
	...
	for(var/obj/effect/landmark/path_waypoint/W in path_nodes)
		...
		path_nodes.Add(W)
		//
		cur_turf = new /turf/simulated/planet/jungle/path(cur_turf)
		...
		while(cur_turf != target_turf)
			...
			if(!istype(cur_turf, /turf/simulated/planet/jungle/))
				cur_dir = get_dir(cur_turf, target_turf)
				cur_turf = get_step(cur_turf, cur_dir)
				continue

			var/turf/simulated/planet/jungle/J = cur_turf
			if(istype(J, /turf/simulated/planet/jungle/impenetrable) || istype(J, /turf/simulated/planet/jungle/water/deep))
				cur_dir = get_dir(cur_turf, target_turf)
				cur_turf = get_step(cur_turf, cur_dir)
				continue

			if(!istype(J, /turf/simulated/planet/jungle/water))
				J = new /turf/simulated/planet/jungle/path(cur_turf)
