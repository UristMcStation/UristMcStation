...
/obj/item/rig_module/actuators /// While on, will dampen the fall from any height and scale power usage accordingly. Enables the user to also jump up 1 z-level.
	...
	desc = "A set of actuators and a linked \"Vaanyari\" speedware chip. They allow the suit to be able to absorb impacts from fatal falls, jump remarkable heights, and move at incredible speeds."
	icon_state = "actuators"

	interface_name = "hardsuit mobility module"
	interface_desc = "A set of actuators and a linked \"Vaanyari\" speedware chip that dampen falls and allow you to dash faster than the eye can see."
...
/obj/item/rig_module/actuators/engage(atom/target, mob/user)
	...
	if (!isturf(H.loc))
		to_chat(H, SPAN_WARNING("You cannot dash out of your current location!"))
		return FALSE

	var/turf/T = get_turf(target)

	if (!T)
		to_chat(H, SPAN_WARNING("You cannot  dash into that location!"))
		return FALSE
	...
	if (T.z != H.z || dist > leapDistance)
		to_chat(H, SPAN_WARNING("You cannot dash at such a distant object!"))
		return FALSE

	if (dist)
		H.visible_message(
			SPAN_WARNING("\The [H]'s suit blitzes at incredible speed towards \the [target]!"),
		...
		H.face_atom(T)
		afterimages.start()
		playsound(H,'sound/effects/basscannon.ogg',35,1)
		var/old_pass_flags = H.pass_flags
		H.pass_flags |= PASS_FLAG_TABLE
		H.status_flags ^= GODMODE
		H.jump_layer_shift()
		H.throw_at(T, leapDistance, 1, H, FALSE, new Callback(src, /obj/item/rig_module/actuators/proc/end_dash, target, user, old_pass_flags))

/obj/item/rig_module/actuators/proc/end_dash(atom/target, mob/user, passed_flags)

	var/mob/living/carbon/human/H = holder.wearer
	H.pass_flags = passed_flags
	H.status_flags ^= GODMODE
	H.jump_layer_shift_end()
	if (H.Adjacent(target))

		if(istype(target, /mob/living/carbon/human))
			H.species.attempt_grab(H, target)
			if(istype(H.get_active_hand(), /obj/item/grab/normal))
				var/obj/item/grab/normal/G = H.get_active_hand()
				G.upgrade()
				visible_message(
					SPAN_WARNING("\The [H] latches onto \the [target]!"),
		...
		if (istype(target, /obj/structure/ladder))
			...
			visible_message(
				SPAN_WARNING("\The [H] zips up \the [target]!"),
				SPAN_WARNING("You quickly jump up \the [target]!")
			)
			ladder.instant_climb(H)

		if(istype(target, /mob/living/exosuit))
			var/mob/living/exosuit/exo = target
			if(!exo.hatch_closed && !LAZYLEN(exo.pilots))
				visible_message(
					SPAN_WARNING("\The [H] dives into \the [target]!"),
