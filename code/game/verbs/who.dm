...
/client/verb/staffwho()   //Chucks all staff in the right category, and then lists them accordingly.
	...
	if(holder)
		for(var/client/C in admins)
			if((R_MOD & C.holder.rights) && (R_HOST ^ C.holder.rights))		//Who shows up in mod rows. Excludes Game Masters and others with all rights.
		...
		for(var/client/C in admins)			//Same as above, but with less detail for non-staff viewers.
			if((R_MOD & C.holder.rights) && (R_HOST ^ C.holder.rights))
				modmsg += "\t[C] is a [C.holder.rank]\n"
				num_mods_online++
