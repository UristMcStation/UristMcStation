...
/obj/structure/pit/use_tool(obj/item/tool, mob/user, list/click_params)
	...
	if (istype(tool, /obj/item/shovel))
		...
		if (!do_after(user, (tool.toolspeed * 5) SECONDS, src) || !user.use_sanity_check(src, tool))
			return TRUE
		user.visible_message(
	...
	if (istype(tool, /obj/item/stack/material/wood))
		...
		if (!do_after(user, 5 SECONDS, src, do_flags = DO_REPAIR_CONSTRUCT) || !user.use_sanity_check(src, tool))
			return TRUE
		if (grave)
			USE_FEEDBACK_FAILURE("\The [src] already has \a [grave].")
			return TRUE
...
/obj/structure/gravemarker/use_tool(obj/item/tool, mob/user, list/click_params)
	...
	if (isHatchet(tool))
		...
		if (!do_after(user, (tool.toolspeed * 2) SECONDS, src) || !user.use_sanity_check(src, tool))
			return TRUE
